#!/usr/bin/env Rscript
cat("[BP-DNAme] Running Rscript 5-DMRs.R...\n")
################################################################################
# BP-DNAme -  Run ENMix::comb-p on results from the EWAS
################################################################################
# Author: Lucile
# Date of creation: September 2021
# Update: November 2021 - Added mean of CpG beta estimates + merged location
# Update: January 2022 - Added seekAnnotation() step
# Note: 
# Run Rscript 3-MLM2-Stage2-EWAS.R before
################################################################################
################################################################################
...
for(f in seq_along( inFiles )){
  ...
  print( inFiles[f] )
  res <- readRDS( inFiles[f] ) %>% dplyr::rename(probe = CpG)
