...
for(w in seq_along(windows)){
  ...
  for(e in seq_along(exposures)){
    ...
    #--------------------------------------------------------------------------#
      ...
      data <- merge(BP, M, by = "id")
      data[,categorical_confounders] <- apply(data[,categorical_confounders],2, as.factor)
      
      mediator.fit <- lm(formula = mediator.formula, data = data)
      
      methC.result <- tapply(dmps$CpG, seq_along(dmps$CpG), simplify = T,
                             FUN = function(cg){
                               
                               CpG <- logit2( Bvalues[cg,] )
                               ...
                               outcome.fit <- lm(formula = outcome.formula, data = data)
                               ...
                               res <- cbind(CpG = cg, 
                                            ...
                                            ACME.estim = x$d.avg, 
                                            ACME.CI = paste0("[", round(x$d.avg.ci[1],5), ";", round(x$d.avg.ci[2],5),"]"),
                                            ACME.pval = x$d.avg.p,
                                            # Direct effects
                                            ADE.estim = x$z.avg,
                                            ADE.CI = paste0("[", round(x$z.avg.ci[1],5), ";", round(x$z.avg.ci[2],5),"]"),
                                            ADE.pval = x$z.avg.p,
                                            # Proportion mediated
                                            PropMediated.estim = x$n.avg,
                                            PropMediated.CI = paste0("[", round(x$n.avg.ci[1],5), ";", round(x$n.avg.ci[2],5),"]"),
                                            PropMediated.pval = x$n.avg.p)
